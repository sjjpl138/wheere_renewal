...
class ReservationServiceTest {
    ...
    public void before() {
        ...
        em.persist(new Reservation(member1, ReservationStatus.CANCEL, "test1", "test2", testDate2, 2));
        em.persist(new Reservation(member1, ReservationStatus.RVW_COMP, "test1", "test2", testDate3, 2));
        em.persist(new Reservation(member1, ReservationStatus.RVW_WAIT, "test1", "test2", testDate4, 2));
        em.persist(new Reservation(member1, ReservationStatus.RVW_COMP, "test1", "test2", testDate5, 2));
        em.persist(new Reservation(member1, ReservationStatus.RVW_COMP, "test1", "test2", testDate6, 2));
        em.persist(new Reservation(member1, ReservationStatus.RESERVED, "test1", "test2", testDate7, 2));
        em.persist(new Reservation(member1, ReservationStatus.CANCEL, "test1", "test2", testDate8, 2));
        em.persist(new Reservation(member1, ReservationStatus.PAID, "test1", "test2", testDate9, 2));
    }
    ...
    public void 예약하기() {
        ...
        List<ReservationBusInfo> busInfo1= new ArrayList<>();
        busInfo1.add(save1);
        SaveResvRequest request = new SaveResvRequest("member1", 1L, 3L, busInfo1, ReservationStatus.PAID, 1250, LocalDate.now());

        Reservation reservation = reservationService.saveReservation(request);

        assertThat(reservation.getMember().getId()).isEqualTo("member1");
        assertThat(reservation.getStartStation()).isEqualTo("조야동");
        assertThat(reservation.getEndStation()).isEqualTo("수성교");
    }
}